...
const add = () => {
  ...
  if (v$.value.$invalid) {
    ...
    store.dispatch("products/create", product);
    console.log(store.state.products.error);
    if (!store.state.products.error) {
      ...
      toast.error(store.state.products.error, {
        duration: duration_time,
        action: [
          {
            text: `OK`,
